...
namespace tests\App\FormationBundle\Repository;

use App\FormationBundle\Test\WebTestCase;
...
class MediaRepositoryTest extends WebTestCase
{
    ...
    public function testGetTopsMedia()
    {
        ...
        $dql = "SELECT m, v FROM App\FormationBundle\Entity\Media m LEFT JOIN m.votes v WHERE m.average IS NOT NULL ORDER BY m.average DESC";
    ...
    public function testGetFlopsMedia()
    {
        ...
        $this->assertNotNull($paginator);
        $this->assertInstanceOf(Paginator::class, $paginator);
        $dql = "SELECT m, v FROM App\FormationBundle\Entity\Media m LEFT JOIN m.votes v WHERE m.average IS NOT NULL ORDER BY m.average ASC";
        $this->assertEquals($dql, $paginator->getQuery()->getDQL());
        $this->assertEquals(10, $paginator->count());
        $this->assertEquals(5, $paginator->getQuery()->getMaxResults());
        $this->assertEquals(0, $paginator->getQuery()->getFirstResult());
